import numpy as np
import cv2 as cv

haar_cascade = cv.CascadeClassifier('haar_face.xml')

#load features and labels array
people = ['Ben Afflek', 'Elton John', 'Jerry Seinfield', 'Mindy Kaling', 'Madonna']
# features = np.load('features.npy', allow_pickle=True) #in this case we are not using these but if needed, allow pickle
# labels = np.load('labels.npy', allow_pickle=True)

#read face_trained.yml file
face_recognizer = cv.face.LBPHFaceRecognizer_create()
face_recognizer.read('face_trained.yml')

img = cv.imread(r'Faces/val/ben_afflek/3.jpg')
gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
cv.imshow('Person', gray)

#Detect the face in the image
faces_rect = haar_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=4)
for (x, y, w, h) in faces_rect:
    faces_roi = gray[y:y+h, x:x+w]

    label, confidence = face_recognizer.predict(faces_roi)
    print(f'Label = {people[label]} with a confidence of {confidence}')
    cv.putText(img, str(people[label]), (20,20), cv.FONT_HERSHEY_COMPLEX, 1.4, (0, 255, 0), 2)
    cv.rectangle(img, (x,y), (x+w, y+h), (0,255,0), thickness=2)

cv.imshow('Person', img)
cv.waitKey(0)

#Note: OpenCV's face recognizer is not the best,
#partially owing to small size of the training set - ideally atleast a couple thousand per clas
#also, it is not a deep learning model, thus unfit for the task
